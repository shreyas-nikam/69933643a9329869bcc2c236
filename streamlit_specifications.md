...
# Streamlit Application Specification: LLM Lab for Financial Analysis

## 1. Application Overview

The **LLM Lab for Financial Analysis** is a Streamlit application designed as a hands-on laboratory for CFA Charterholders and Investment Professionals. Its primary purpose is to explore and apply Large Language Models (LLMs) to critical financial analysis tasks, specifically earnings call summarization and financial concept explanation, while emphasizing compliance, hallucination mitigation, and return on investment (ROI).

The application simulates a real-world workflow where an investment professional leverages LLMs to enhance efficiency while rigorously adhering to ethical and professional standards, particularly **CFA Standard V(A): Diligence and Reasonable Basis**. The user will engage with different prompt engineering strategies, understand token management, extract structured data, calibrate LLM behavior, and conduct a systematic hallucination audit. The overarching story flow guides the user from basic LLM interaction to a comprehensive evaluation of LLM capabilities and limitations in a financial context.

The application will be structured into several logical "pages" (simulated via sidebar navigation) that mirror the progressive learning and application of LLMs in financial analysis, demonstrating how these tools integrate into an investment professional's daily tasks.
...
`st.session_state` will be used to preserve application state across user interactions and page navigations.
...
if 'llm_model' not in st.session_state:
    st.session_state.llm_model = 'gpt-4o' # Default LLM model
if 'selected_transcript' not in st.session_state:
    st.session_state.selected_transcript = list(transcripts.keys())[0] if transcripts else None # Default transcript
...
if 'summaries' not in st.session_state:
    st.session_state.summaries = {} # Stores {transcript_name: {strategy: {'summary': str, 'usage': dict, 'audit': dict, 'h_rate': float}}}
if 'json_metrics' not in st.session_state:
    st.session_state.json_metrics = {} # Stores {transcript_name: dict}
if 'comparison_data' not in st.session_state:
    st.session_state.comparison_data = pd.DataFrame() # Stores prompt strategy comparison data
if 'temp_consistency_results' not in st.session_state:
    st.session_state.temp_consistency_results = {} # Stores temperature consistency Jaccard similarities
if 'summarization_temperature' not in st.session_state:
    st.session_state.summarization_temperature = 0.3 # Recommended default for narrative prose
if 'json_extraction_temperature' not in st.session_state:
    st.session_state.json_extraction_temperature = 0.0 # Non-negotiable for factual extraction
if 'audit_target_summary' not in st.session_state:
    st.session_state.audit_target_summary = 'structured' # Default summary to audit
```

**Updates:**
*   `st.session_state.current_page`: Updated via sidebar `st.selectbox`.
*   `st.session_state.llm_model`: Updated via sidebar `st.selectbox`.
*   `st.session_state.selected_transcript`: Updated via sidebar `st.selectbox`.
*   `st.session_state.concept_query`: Updated when user types into `st.text_area`.
*   `st.session_state.explanation_response`: Updated after `explain_financial_concept` is called.
*   `st.session_state.summaries`: Updated after `summarize_naive`, `summarize_with_strategy`, or `hierarchical_summarize` are called. Stores summary, token usage, and optionally audit results for each.
*   `st.session_state.json_metrics`: Updated after `extract_metrics` is called.
*   `st.session_state.comparison_data`: Updated after the "Run Prompt Strategy Comparison" button is clicked, populating it with a DataFrame.
*   `st.session_state.temp_consistency_results`: Updated after the "Run Temperature Consistency Experiment" button is clicked.
*   `st.session_state.summarization_temperature`: Updated via `st.slider`.
*   `st.session_state.json_extraction_temperature`: Updated via `st.slider`.
*   `st.session_state.audit_target_summary`: Updated via `st.selectbox`.

**Reads:**
All page content and widget default values read from `st.session_state` to maintain persistence. For example, when generating a summary, `st.session_state.selected_transcript` and `st.session_state.llm_model` are read. When displaying audit results, `st.session_state.summaries` and `st.session_state.audit_target_summary` are read.

### Application Layout and Flow

The application will use a sidebar for global settings and page navigation, and the main area for content specific to the selected page.

```python
st.set_page_config(layout="wide", page_title="LLM Lab for Financial Analysis")
...
if st.session_state.current_page == "Application Overview":
    # --- Markdown from source.py: Revolutionizing Financial Analysis ---
    ...
    # --- Markdown from source.py: 1. Setting Up the Environment ---
    ...
    # --- Markdown from source.py: 2. LLM Explanatory Power ---
    st.markdown(f"""
    ...
    if st.session_state.explanation_response:
        ...
        # --- Markdown from source.py (explanation of LLM's explanation) ---
        st.markdown(f"""
    ...
    with tab1:
        ...
        # --- Markdown from source.py: 3. Streamlining Workflow: Summarizing Earnings Calls (Naive Approach) ---
        st.markdown(f"""
        ...
        if st.button("Generate Naive Summary", key="naive_summary_btn"):
            with st.spinner("Generating naive summary..."):
                try:
                    ...
                    st.session_state.summaries[st.session_state.selected_transcript]['naive'] = {'summary': summary, 'usage': None} # Usage not returned by naive fn
                except Exception as e:
        ...
        if st.session_state.selected_transcript in st.session_state.summaries and 'naive' in st.session_state.summaries[st.session_state.selected_transcript]:
            ...
            # --- Markdown from source.py (explanation of naive summary) ---
            st.markdown(f"""
    ...
    with tab2:
        ...
        # --- Markdown from source.py: 4. Enhancing Precision: Structured Summaries & Anti-Hallucination Guardrails ---
        st.markdown(f"""
        ...
        *   **Specificity Instructions:** Directives like "Quote specific numbers exactly as stated" or "distinguish between RAISED, LOWERED, MAINTAINED guidance" enforce numerical precision and accurate characterization of management commentary, minimizing ambiguous paraphrasing.
        ...
        if st.session_state.selected_transcript in st.session_state.summaries and selected_strategy in st.session_state.summaries[st.session_state.selected_transcript]:
            ...
            # --- Markdown from source.py (explanation of structured summary) ---
            st.markdown(f"""
    ...
    with tab3:
        ...
        # --- Markdown from source.py: 5. Managing Scale: Handling Long Transcripts with Token Management & Chunking ---
        st.markdown(f"""
        ...
        *   **Hierarchical Summarization:** Summarizing individual chunks first, then merging these summaries into a final, comprehensive brief. This ensures no part of the original document is overlooked.
        ...
        if st.button("Generate Hierarchical Summary", key="hierarchical_summary_btn"):
            with st.spinner("Generating hierarchical summary... This might take longer for long transcripts."):
                try:
                    ...
                    if st.session_state.selected_transcript not in st.session_state.summaries:
                        st.session_state.summaries[st.session_state.selected_transcript] = {}
                    st.session_state.summaries[st.session_state.selected_transcript]['hierarchical'] = {'summary': final_summary, 'usage': None} # Usage for hierarchical is complex, not stored directly
        ...
        if st.session_state.selected_transcript in st.session_state.summaries and 'hierarchical' in st.session_state.summaries[st.session_state.selected_transcript]:
            ...
            # --- Markdown from source.py (explanation of hierarchical summary) ---
            st.markdown(f"""
    ...
    with tab4:
        ...
        # --- Markdown from source.py: 6. Extracting Specifics: Structured JSON for Key Financial Metrics ---
        st.markdown(f"""
        ...
        *   **Clear Field Definitions:** Explicitly defining required JSON fields and their expected data types.
        ...
        st.session_state.json_extraction_temperature = st.slider(
            ...
            disabled=True if st.session_state.json_extraction_temperature != 0.0 else False # enforce 0.0 for demonstration
        )
        ...
        if st.session_state.selected_transcript in st.session_state.json_metrics:
            ...
            # --- Markdown from source.py (explanation of JSON extraction) ---
            st.markdown(f"""
    ...
    with tab5:
        ...
        # --- Markdown from source.py: 8. Optimizing & Comparing: Prompt Strategies and Temperature Calibration ---
        st.markdown(f"""
        ...
        *   **Temperature Calibration:** The `temperature` parameter controls the randomness of the LLM's output. `$\\tau=0.0$` leads to deterministic, factual output (ideal for numerical extraction), while higher temperatures introduce more creativity (suitable for qualitative prose summaries, but risky for facts).
        ...
        st.markdown(r"$$ P(\text{{token}}_t | \text{{context}}) = \frac{{\exp(z_t/\tau)}}{{\sum_j \exp(z_j/\tau)}} $$")
        ...
        if st.button("Run Prompt Strategy Comparison", key="run_comparison_btn"):
            with st.spinner("Running comparison across prompt strategies... This may take a moment."):
                ...
                # key_facts_transcript is defined in source.py
                ...
                st.session_state.comparison_data = pd.DataFrame(comparison).T.astype(str) # Convert to string for consistent display of mixed types
            ...
            with st.spinner("Running temperature consistency experiment... This involves multiple LLM calls."):
                ...
                for temp in temperatures:
                    try:
                        ...
                        temperature_consistency_results[temp] = 0.0 # Assign 0 if error
    ...
    # --- Markdown from source.py: 7. Mitigating Risk: The Hallucination Audit & Compliance Safeguard ---
    st.markdown(f"""
    ...
    *   **Completeness Check:** Ensuring key financial topics (revenue, earnings, margin, guidance, outlook) are mentioned in the summary.
    ...
    st.session_state.audit_target_summary = st.selectbox(
        ...
        index=list(available_summaries.keys()).index(st.session_state.audit_target_summary.replace('_', ' ').title()) if st.session_state.audit_target_summary.replace('_', ' ').title() in available_summaries.keys() else 0,
    ...
    if st.button(f"Run Audit for {st.session_state.audit_target_summary}", key="run_audit_btn"):
        with st.spinner("Running hallucination audit..."):
            try:
                ...
                if st.session_state.selected_transcript not in st.session_state.summaries:
                     st.session_state.summaries[st.session_state.selected_transcript] = {} # Should already exist, but for safety
    ...
    if st.session_state.selected_transcript in st.session_state.summaries and selected_summary_key in st.session_state.summaries[st.session_state.selected_transcript] and 'audit' in st.session_state.summaries[st.session_state.selected_transcript][selected_summary_key]:
        ...
        # --- Markdown from source.py (explanation of hallucination audit) ---
        st.markdown(f"""
    ...
    # --- Markdown from source.py: 9. Quantifying Value: ROI and Ethical Considerations ---
    st.markdown(f"""
    ...
    if not st.session_state.comparison_data.empty and 'structured' in st.session_state.comparison_data.index:
        ...
        if st.session_state.selected_transcript in st.session_state.summaries and 'structured' in st.session_state.summaries[st.session_state.selected_transcript] and st.session_state.summaries[st.session_state.selected_transcript]['structured']['usage']:
            ...
            # Try to get hallucination rate if audit was run
    ...
    total_transcripts_processed = len(transcripts) # Number of sample transcripts
    total_demo_cost = sum(float(v['cost']) for k, v in st.session_state.comparison_data.to_dict('index').items() if v['cost'] != 'N/A') if not st.session_state.comparison_data.empty else (estimated_cost_per_transcript * total_transcripts_processed)
    ...
    *   Importance of documenting the audit process for transparency and accountability.
    ...
    # --- V4 Token Cost Breakdown (Placeholder - requires aggregating actual token usage) ---
    ...
    for transcript_content in transcripts.values():
        try:
            # Re-run a dummy extraction to get token estimates without actually calling LLM API
            # This is illustrative, a real app would cache usage
            extraction_messages = [
            ...
            # Estimating tokens without calling actual API for illustrative purpose if needed
            enc = tiktoken.encoding_for_model(st.session_state.llm_model)
            total_input_tokens += len(enc.encode(str(extraction_messages))) # Approximate
            total_output_tokens += 500 # Assume ~500 output tokens for JSON
        except Exception:
            pass # Ignore if transcript not available or encoding fails
    ...
    # --- Markdown from source.py (explanation of ROI and Ethical Brief) ---
    st.markdown(f"""
